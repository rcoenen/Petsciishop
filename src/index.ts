...
async function main() {
  ...
  const mobileShareRoute = hasShareHash && isMobileDevice();

  const store = configureStore();
  const dispatch = store.dispatch as any;

  // Hash share URL has startup priority on desktop. On mobile the React app
  // renders the read-only share viewer directly from the current hash.
  if (hasShareHash && !mobileShareRoute) {
    try {
