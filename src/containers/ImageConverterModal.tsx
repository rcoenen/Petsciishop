...
function resetOutputModes(settings: ConverterSettings): ConverterSettings {
  return {
    ...settings,
    outputStandard: settings.outputStandard ?? MODAL_OUTPUT_DEFAULTS.outputStandard,
    outputEcm: settings.outputEcm ?? MODAL_OUTPUT_DEFAULTS.outputEcm,
    outputMcm: settings.outputMcm ?? MODAL_OUTPUT_DEFAULTS.outputMcm,
  };
}
...
function loadSettings(): ConverterSettings {
  try {
    ...
    if (saved) {
      const parsed = JSON.parse(saved) as Partial<ConverterSettings> & { __v?: number };
      const migrated = parsed.__v === STORAGE_VERSION
        ? parsed
        : { ...parsed, includeTypographic: true };
      return normalizeOutputModes(resetOutputModes({ ...CONVERTER_DEFAULTS, ...migrated }));
    }
  } catch { /* ignore */ }
...
function saveSettings(settings: ConverterSettings) {
  try {
    localStorage.setItem(
      STORAGE_KEY,
      JSON.stringify({
        ...normalizeOutputModes(resetOutputModes(settings)),
        __v: STORAGE_VERSION,
      })
    );
  } catch { /* ignore */ }
...
function buildDirtyModes(
  ...
  return PREVIEW_MODE_ORDER.filter(mode => {
    if (mode === 'standard' && !settings.outputStandard) return false;
...
export default function ImageConverterModal() {
  ...
  const handleImport = useCallback((result: ConversionResult) => {
    const fb = resultToFramebuf(result, {
      ...
      description: 'Generated by TRUSKI3000™',
    });
  ...
  return (
    <Modal showModal={show} width={1500}>
      <div className={styles.container}>
        ...
        <div className={styles.topRow}>
          ...
          <fieldset className={`${styles.settings} ${controlsDisabled ? styles.controlsDisabled : ''}`} disabled={controlsDisabled}>
            ...
            <div className={styles.settingsRow}>
              <label>CSF Penalty:</label>
              <input
                type="range" min="0" max="20" step="0.5"
                value={settings.csfWeight}
                onChange={(e) => updateSetting('csfWeight', parseFloat(e.target.value))}
              />
              <span className={styles.value}>{settings.csfWeight.toFixed(1)}</span>
            </div>

            <div className={styles.settingsRow}>
              <label>Text Glyphs:</label>
              <label className={styles.inlineCheckbox}>
                <input
                  type="checkbox"
                  checked={settings.includeTypographic}
                  onChange={(e) => updateSetting('includeTypographic', e.target.checked)}
                />
                <span>Allow letters, digits, and punctuation</span>
              </label>
            </div>
          </fieldset>
