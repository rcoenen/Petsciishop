...
import { colorIndexToCssRgb, colorPalettes } from '../utils';
...
const Title: React.FC<{children?: React.ReactNode}> = ({children}) => <h4>{children}</h4>
const ErrorMsg: React.FC<{ msg: string }> = ({ msg }) => <div className={classnames(styles.error, styles.title)}>Error: <span className={classnames(styles.error, styles.msg)}>{msg}</span></div>
const Text: React.FC<{children?: React.ReactNode}> = ({children}) => <div className={styles.text}>{children}</div>
...
async function decodePng(buffer: ArrayBuffer): Promise<PngData> {
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      resolve({ width: img.width, height: img.height, data: new Uint8Array(imageData.data.buffer) });
    };
    img.onerror = () => {
      URL.revokeObjectURL(url);
...
function petsciify(png: PngData|undefined, colorPalettes: Rgb[][], charset: string) {
  if (!png) {
    return undefined;
  }
  const petscii = png2pet.png2petscii({
    ...
    fontBits: new Uint8Array(getROMFontBitsMemoized(charset).bits)
  });
  return petscii;
}
